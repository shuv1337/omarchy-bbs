...
CREATE TABLE IF NOT EXISTS threads (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,user_id BIGINT UNSIGNED NOT NULL,title_enc TEXT NOT NULL,body_enc MEDIUMTEXT NOT NULL,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
CREATE TABLE IF NOT EXISTS replies (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,thread_id BIGINT UNSIGNED NOT NULL,user_id BIGINT UNSIGNED NOT NULL,body_enc MEDIUMTEXT NOT NULL,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(thread_id) REFERENCES threads(id) ON DELETE CASCADE,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
SQL); }
...
function page(string $title,array $u,string $content): never {
  ...
  echo '<!doctype html><html lang=en><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>'.e($title).' · Omarchy BBS</title><style>'.$css.'</style></head><body><main><header class=mast><div><div class=brand>OMARCHY//BBS</div><div class=tag>local machines. shared frequency.</div></div><div class=meta>linked as <span class=badge>@'.e($u['handle']).'</span></div></header><nav class=nav><a href=/>[ threads ]</a><a href=/new>[ new transmission ]</a><a href=/logout>[ disconnect ]</a></nav>'.$content.'<footer>ACCESS GRANTED VIA OMARCHY DEVICE CREDENTIAL</footer></main></body></html>'; exit;
}
